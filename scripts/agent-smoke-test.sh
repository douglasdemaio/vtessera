...
# Flatpak GUI with "Accept workloads" on and mode set to "free"), OR — when
# VTESSERA_QUEUE is set — an outbound-only node behind a coordinator queue
# (P1.7): the smoke test then enqueues through the queue instead of HTTP.
#
# Env overrides:
#   VTESSERA_NODE_URL  node base URL (default http://127.0.0.1:8402)
#   VTESSERA_QUEUE     path to the coordinator's EndpointAddr JSON (queue mode)
#   VTESSERA_AGENT_ID  agent identity header (default smoke-test)
set -euo pipefail

NODE_URL="${VTESSERA_NODE_URL:-http://127.0.0.1:8402}"
QUEUE="${VTESSERA_QUEUE:-}"
...
if [ -n "$QUEUE" ]; then
    echo "Queue: $QUEUE (queue rendezvous)"
else
    echo "Node: $NODE_URL"
fi
echo
...
echo "1. Health check"
if [ -n "$QUEUE" ]; then
    if vtessera-agent --queue "$QUEUE" health >/dev/null 2>&1; then
        pass "coordinator dialed (queue rendezvous reachable)"
    else
        fail "queue not reachable at $QUEUE"
        echo; echo "Result: $FAIL failed"; exit 1
    fi
elif resp=$(curl -sf "$NODE_URL/healthz" 2>/dev/null); then
...
echo "2. Offer"
if [ -n "$QUEUE" ]; then
    if vtessera-agent --queue "$QUEUE" offer >/dev/null 2>&1; then
        pass "queue offer surface present"
    else
        fail "could not fetch offer through the queue"
    fi
elif offer=$(curl -sf "$NODE_URL/offer" 2>/dev/null); then
...
if [ -n "$QUEUE" ]; then
    job_file="$(mktemp)"
    printf '{"job_id":"%s","image":"busybox","command":["echo","hello from smoke test"],"env":[],"devices":{"class":{"kind":"cpu"},"vcpus":1,"mem_kb":65536,"min_vram_mb":0},"network":"none","max_duration_secs":60}' "$JOB_ID" > "$job_file"
    if queued=$(vtessera-agent --queue "$QUEUE" submit --job "$job_file" 2>&1); then
        pass "job enqueued at coordinator"
        echo "       $queued"
    else
        fail "enqueue failed: $queued"
    fi
    rm -f "$job_file"
else
...
# 4. MCP endpoint (HTTP mode only; queue mode has no inbound listener)
echo "4. MCP discover"
if [ -n "$QUEUE" ]; then
    echo "  - skipped (queue-mode: no inbound HTTP listener)"
else
...
# 5. A2A agent card (HTTP mode only)
echo "5. A2A agent card"
if [ -n "$QUEUE" ]; then
    echo "  - skipped (queue-mode: no inbound HTTP listener)"
elif card=$(curl -sf "$NODE_URL/.well-known/agent.json" 2>/dev/null); then
