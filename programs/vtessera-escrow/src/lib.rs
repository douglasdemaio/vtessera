...
// Program ID. This is the on-chain address the program is deployed at.
// Generated by `anchor build` / `cargo build-sbf` (see
// `programs/target/deploy/vtessera_escrow-keypair.json`). Replace if you
// redeploy under a fresh keypair.
...
pub mod vtessera_escrow {
    ...
    ///
    /// **DEVNET-STUB BEHAVIOUR (ROADMAP §0).** The production design
    /// (ROADMAP §4b) swaps `earned` stablecoin → HNT via Jupiter,
    /// Pyth-guarded, burns `DRAFT_BURN_BPS`, and pays the seller in HNT.
    /// This skeleton ships the **stub**: the earned slice is paid to the
    /// seller in the **same stablecoin** the buyer deposited. No swap,
    /// no oracle, no burn. The flow shape (pro-rata split, refund-by-`f`,
    /// contract.finalized = true) is identical, so the devnet test can
    /// exercise the full lifecycle end-to-end while the swap is built.
    ///
    /// When Jupiter + Pyth land, this function changes; the IX signature
    /// and the buyer-side semantics do not.
    ///
    /// - `earned = f × price` → seller_stablecoin_ata (STUB: should swap to HNT)
    /// - `refund = (1 − f) × price` → buyer_stablecoin_ata
    ///
    /// `f_micros` is `f` scaled by 1_000_000 — i.e. `f = 0.5` ⇒
    /// `f_micros = 500_000`. Fixed-point integer keeps every step of the
    /// split deterministic and avoids floating-point on-chain.
    pub fn finalize_pro_rata(ctx: Context<FinalizePro>, f_micros: u32) -> Result<()> {
        ...
        // ---- Earned slice (STUB: stablecoin → seller; production: swap → HNT → seller) ----
        if earned > 0 {
            let cpi_accounts = Transfer {
...
pub struct FinalizePro<'info> {
    ...
    /// multisig. For devnet skeleton testing, any signer the off-chain
    /// orchestrator trusts.
    ...
    /// Seller's stablecoin ATA. **STUB**: production replaces this with
    /// the seller's HNT ATA and a Jupiter swap into HNT. Until that
    /// lands the seller is paid in the stablecoin the buyer deposited.
    #[account(
        mut,
...
/// Emitted by `finalize_pro_rata` so off-chain consumers (settlement
/// dashboard, future indexers) can reconcile the on-chain split without
/// re-parsing the contract account.
#[event]
