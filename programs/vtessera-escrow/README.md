...
Devnet: **`8UJy6B2ZX3swc6XLgGzWeEfcrP7ujZyBcFrKfp5YkA47`**
(ProgramData `D2J1NTjx3r44HJdJL9upESKk3oaJ5ZEXMqhTsZGqfHjQ`,
config PDA `6UGiFSD76PSUo1SUJguGnUEv86AqtgoEdSqXDyp9hFhw`).

Rotated from the retired `D4iX…` on 2026-09-17: the audited build
outgrew the old ProgramData (334,296 B vs 292,120 B) and
`BPFLoaderUpgradeable` cannot resize program data, so the program moved
to a freshly generated deploy keypair — the same fresh-keypair flow used
at first mainnet deploy. Reproducible SHA-256 (`solana-verify build`,
0.5.1): `5d7f6dfd40389cb7a7beab2499516fe92607cf74f2e7b3764dd1e2e865a68a56`,
pinned in `DEPLOYED_SHA256.txt`.

The keypair lives at `programs/target/deploy/vtessera_escrow-keypair.json`;
the same address is declared in `src/lib.rs` (`declare_id!`) and
`programs/Anchor.toml`. On first mainnet deploy a fresh keypair is
generated and these three sites get updated together.
...
at `8UJy6B2ZX3swc6XLgGzWeEfcrP7ujZyBcFrKfp5YkA47`. The production
