...
| Aggregated "go over index" overview | `vtessera-agent overview` aggregates every visible offer (index + marketplace, free + paid, claimed + unclaimed) with price/reach/dial/claim/candidates/heartbeat, plus normalized `--json` and `--mode`/`--device`/`--available` filters | `crates/agent-cli/src/main.rs` (`overview`, `merge_offers`, `overview_row`, `overview_json_row`); spec `docs/design/specs/2026-09-10-overview-go-over-index-design.md` |
