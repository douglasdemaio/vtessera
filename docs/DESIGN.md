...
| `crates/executor` | Module 1 — execution + accelerators | shipped (CPU backends: noop-cpu, local-cpu, cloud-hypervisor; Kata Containers backend) |
| `crates/offer` | Module 2 — signed machine-readable offers | shipped |
| `crates/node-api` | Module 2 — x402 / MCP HTTP surface (feature-gated) | shipped (off-chain Solana payment verification) |
...
| `crates/marketplace-server` | Private/enterprise mode — receipt store | shipped |
| `crates/vtessera-config` | Private/enterprise mode — config wizard | shipped |
| `crates/metering-sidecar` | Module 1 — Kata guest-side metering | shipped |
...
interface; the heavy implementation work (DCGM telemetry, confidential
GPU, etc.) lands per the ROADMAP milestones. "Shipped" means
...
isolation, `kata-cloud-hypervisor` for Kata Containers). On-chain payment
verification is wired via off-chain Solana RPC (`--rpc-url`), confirming
transaction finalization, escrow account involvement, and sufficient
token transfer amount before running paid jobs.
